...
def reflect_on_result(task_description: str, result: str) -> dict:
    ...
    import json
    messages = [
        {"role": "user", "content": REFLECTOR_PROMPT.format(
            task_description=task_description,
            result=result,
        )}
    ]
    response = get_llm_response(messages, temperature=0.1)  # 后端 低温度保证判断一致
    ...
    try:
        ...
        return {"pass": True, "comment": "格式解析异常，默认通过"}  # 后端 解析失败不免杀
...
def reflect_and_fix(subtask: dict, max_retries: int = 2) -> tuple[str, int]:
    # 后端 反思+修正循环，返回 (最终结果, 重试次数)
    result = subtask.get("result", "")
    task_desc = subtask.get("description", "")

    for attempt in range(max_retries):
        review = reflect_on_result(task_desc, result)  # 后端 审查当前结果
        if review.get("pass", False):
            return result, attempt  # 后端 审查通过，直接返回
