...
class SQLMemory:
    ...
    def __init__(self):
        # 后端 psycopg v3 连接池（避免 psycopg2 在 Windows GBK 编码下的 UnicodeDecodeError）
        self._pool = ConnectionPool(
            conninfo=f"host={settings.pg_host} port={settings.pg_port} dbname={settings.pg_database} "
    ...
    def _init_tables(self) -> None:
        ...
        cur.close()
        conn.autocommit = True  # 后端 归还前重置，避免回滚警告
        self._put_conn(conn)
